...
export default function Authenticators() {
    ...
    const filteredAuthenticators = useMemo(() => {
							...
							<TableCell align="right">
								<Tooltip title="View authenticator">
									<IconButton
										...
										size="small"
										onClick={() => navigate(`/authenticators/${authenticator.type}/${authenticator.name}`)}
									>
		...
		async function handleToggleAuthenticator(authenticator, checked) {
			try {
				const updatedAuthenticator =  await authenticatorsService.update(checked, authenticator.type, authenticator.name)
		...
		async function handleDeleteAuthenticator(authenticator) {
			try {
				await authenticatorsService.delete(authenticator.type, authenticator.name)
        ...
        <Box sx={{ py: 4 }}>
            <Container>
                ...
                <Stack spacing={2} sx={{ mt: 3 }}>
                    ...
                    {!loading && !error && authenticators.length > 0 && filteredAuthenticators.length === 0 && (
                        <Alert severity="info">No authenticators match your search.</Alert>
                    )}

                    {!loading && !error && filteredAuthenticators.length > 0 && (
                        <TableContainer component={Paper}>
                            <Table sx={{ minWidth: 650 }} size="small" aria-label="authenticators table">
                                <TableHead>
                                    <TableRow>
